...
$userStatement = $database->prepare(
    'SELECT u.username, up.display_name, up.bio, up.avatar_url, up.cover_url, up.profile_visibility,
            up.who_can_message, up.gaming_style, up.preferred_voice_chat,
            up.email_notifications, up.push_notifications, up.social_notifications,
...
$platformRows = $database->prepare('SELECT platform FROM user_platforms WHERE user_id = :user_id ORDER BY platform');
$platformRows->execute(['user_id' => $userId]);
$selectedPlatforms = $platformRows->fetchAll(PDO::FETCH_COLUMN, 0);

$contentRows = $database->prepare('SELECT content_type FROM user_content_preferences WHERE user_id = :user_id ORDER BY content_type');
$contentRows->execute(['user_id' => $userId]);
$selectedContent = $contentRows->fetchAll(PDO::FETCH_COLUMN, 0);
...
$tab = isset($_GET['tab']) ? (string) $_GET['tab'] : 'account';
$validTabs = ['account', 'privacy', 'notifications', 'security'];
...
$sessionRows = $database->prepare(
    'SELECT id, ip_address, user_agent, created_at, expires_at FROM sessions WHERE user_id = :user_id ORDER BY created_at DESC'
);
$sessionRows->execute(['user_id' => $userId]);
$activeSessions = $sessionRows->fetchAll();

$loginHistory = $database->prepare(
    'SELECT device_name, ip_hash, user_agent, action, created_at FROM user_login_history WHERE user_id = :user_id ORDER BY created_at DESC LIMIT 20'
);
$loginHistory->execute(['user_id' => $userId]);
$loginEntries = $loginHistory->fetchAll();

$errorMessage = null;
if (isset($_GET['error']) && $_GET['error'] === '1') {
    $errorMessage = 'We could not update your profile because image processing is not available on this server. Please install the GD or Imagick extension or use external image URLs.';
}

$defaultAvatar = appUrl('assets/icons/profile.png');
$defaultCover = appUrl('assets/img/cover-default.jpg');
?>
<!doctype html>
<html lang="en">
<head>
    ...
    <title>Account settings | GamersHUB</title>
    ...
    <link rel="stylesheet" href="<?= htmlspecialchars(appUrl('assets/css/account.css'), ENT_QUOTES, 'UTF-8') ?>">
    <script src="<?= htmlspecialchars(appUrl('assets/js/account.js'), ENT_QUOTES, 'UTF-8') ?>" defer></script>
</head>
<body>
<div class="app-shell">
                        ...
                        <div class="security-box">
                            ...
                            <div class="session-list">
                                <?php foreach ($activeSessions as $session): ?>
                                    ...
                                    $ipHash = $session['ip_address'] ? hash('sha256', $session['ip_address']) : 'hidden';
                                    ?>
                                    <div class="session-item <?= $isCurrentSession ? 'current' : '' ?>">
                                        ...
                                        <div class="session-meta">
                                            <span><?= htmlspecialchars($ipHash, ENT_QUOTES, 'UTF-8') ?></span>
