...
$tab = in_array((string) ($_POST['tab'] ?? 'account'), ['account', 'privacy', 'notifications', 'security'], true)
    ? (string) $_POST['tab']
    : 'account';
...
try {
    ...
    $loginHistory = $database->prepare(
        'INSERT INTO user_login_history (user_id, device_name, ip_hash, user_agent, action, created_at) VALUES (:user_id, :device_name, :ip_hash, :user_agent, :action, NOW())'
    );
